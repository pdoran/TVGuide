var dataReady = function() {
	...
	var User = function() {
		...
		self.toggleEpisode = function(episode) {
			if(self.likes().indexOf(episode)===-1) {
				self.addEpisode(episode);
			} else {
				self.removeEpisode(episode);
			}
		};
		self.addEpisode = function(episode) {
			self.likes.push(episode);
		};	

		self.removeEpisode = function(episode) {
			self.likes.remove(episode);
		};
	...
	var viewModelObject = function(shows) {
		...
		self.userLikeCount = ko.computed(function() {
			return self.user().likes().length;
		}, self);
		...
		self.showUserFavorites = function() {
			$("#userFavoriteModal").modal("show");
		}
	};
